...
class FilesImagesApi
{
    ...
    {
        list($response) = $this->deleteBaseAssetWithHttpInfo($path, $contentType);
        return $response;
    }
    ...
    {
        $request = $this->deleteBaseAssetRequest($path, $contentType);
     ...
     */
    public function deleteBaseAssetAsync($path, string $contentType = self::contentTypes['deleteBaseAsset'][0])
    {
        return $this->deleteBaseAssetAsyncWithHttpInfo($path, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function deleteBaseAssetAsyncWithHttpInfo($path, string $contentType = self::contentTypes['deleteBaseAsset'][0])
    {
        $returnType = 'object';
        $request = $this->deleteBaseAssetRequest($path, $contentType);
     ...
     */
    public function deleteBaseAssetRequest($path, string $contentType = self::contentTypes['deleteBaseAsset'][0])
    {

        // verify the required parameter 'path' is set
        if ($path === null || (is_array($path) && count($path) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $path when calling deleteBaseAsset'
            );
        }
        if (!preg_match("/^\/(images|files){1}\/[0-9]{4}\\-[0-9]{2}\/./", $path)) {
            throw new \InvalidArgumentException("invalid value for \"path\" when calling FilesImagesApi.deleteBaseAsset, must conform to the pattern /^\/(images|files){1}\/[0-9]{4}\\-[0-9]{2}\/./.");
        }
        

        $resourcePath = '/api/v2.1/dtable/app-asset/';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
     ...
     * @throws \SeaTable\Client\ApiException on non-2xx response or if the response body is not in the expected format
     * @throws \InvalidArgumentException
     * @return object|\SeaTable\Client\File\GetFileDownloadLink404Response
     */
     ...
     * @throws \SeaTable\Client\ApiException on non-2xx response or if the response body is not in the expected format
     * @throws \InvalidArgumentException
     * @return array of object|\SeaTable\Client\File\GetFileDownloadLink404Response, HTTP status code, HTTP response headers (array of strings)
     */
    public function getFileDownloadLinkWithHttpInfo($path, string $contentType = self::contentTypes['getFileDownloadLink'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                ...
                case 404:
                    return $this->handleResponseWithDataType(
                        '\SeaTable\Client\File\GetFileDownloadLink404Response',
                        $request,
                        $response,
                    );
            }
     ...
     * @param  string|null $replace Do you want to overwrite a file/image with the same name? &#x60;0&#x60; - No, &#x60;1&#x60; - Yes. Optional. &#x60;0&#x60; by default. If existing file is not overwritten, the uploaded file will be renamed as &#x60;filename(1).xxx&#x60;. (optional)
