...
Once you have created a single/multiple select column, use this request to add options for it.  In the request body:  `table_name` is the name of the table;  `column` is the name or the `key` of the single/multiple select column;  `options` is an array containing the label (`name`), option color (`color`) and the color of the arrow (`textColor`):  ![Options](https://seatable.com/openapi/selectoptioncolor.png)
...
```php
insertColumn($base_uuid, $insert_column_request): \SeaTable\Client\Base\UpdateColumn200Response
```
...
[**\SeaTable\Client\Base\UpdateColumn200Response**](../Model/UpdateColumn200Response.md)
...
```php
updateColumn($base_uuid, $update_column): \SeaTable\Client\Base\UpdateColumn200Response
```
